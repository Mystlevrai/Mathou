"""Config du worker anime-sama/nakanime. Toutes les valeurs viennent de anime-worker/.env."""
...
def _json_list(name: str) -> list[str]:
    ...
    try:
        ...
        # pas du JSON : on accepte une liste separee par des espaces
        return raw.split()
    if not (isinstance(val, list) and all(isinstance(x, str) for x in val)):
        raise RuntimeError(f"{name} doit etre un tableau JSON de chaines, ex : [\"--player\",\"sibnet\"]")
    return val
...
class Config:
    ...
    def load() -> "Config":
        ...
        return Config(
            ...
            # dossier de build LOCAL distinct de celui du worker mathou (evite une
            # ecriture concurrente sur le meme dossier) ; les deux se synchronisent
            # quand meme vers la MEME destination B2 (catalogue partage)
            catalog_local=Path(os.getenv("CATALOG_LOCAL", r"C:\mathou\catalog-anime")).resolve(),
