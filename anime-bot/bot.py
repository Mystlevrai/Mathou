"""anime-bot — interface Discord pour Anime-Downloader (anime-sama/nakanime).
Bot Discord separe de mathou (jeton different), worker separe (port different),
mais MEME base/catalogue que mathou (cf anime-worker/.env DB_PATH)."""
...
# le panel /admin vit sur le worker MATHOU (base partagee -> un seul panel pour les 2 outils)
MATHOU_ADMIN_URL = os.getenv("MATHOU_ADMIN_URL", "").strip()
...
@client.tree.command(name="admin", description="Lien du panel admin du catalogue (prive, partage avec mathou)")
async def admin(interaction: discord.Interaction) -> None:
    if not is_allowed(interaction.user):
        await interaction.response.send_message("Non autorise.", ephemeral=True)
        return
    if not MATHOU_ADMIN_URL:
        await interaction.response.send_message("MATHOU_ADMIN_URL non configure.", ephemeral=True)
        return
    await interaction.response.send_message(MATHOU_ADMIN_URL, ephemeral=True)
